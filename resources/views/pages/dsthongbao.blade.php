            ...
            <div class="bg-light rounded p-2">
                <div class="row">
                    <div class="col-12 py-2" style="margin-top: 35px;">
                        ...
                        <a href="{{URL::to('/Formthemthongbao')}}" style="float: right;" class="rounded-3 btn btn-primary btn-lg">Thêm thông báo</a>
                    </div>
                    <div class="container ps-5">
                        <div class="row">
                            <div class="col-lg-12">
                                <?php
                                    $thongbaothanhcong = session("themthongbaothanhcong");
                                    if ($thongbaothanhcong) {
                                        echo '<span style = "color: red" class ="fs-5">' . $thongbaothanhcong . '</span>';
                                        session(["themthongbaothanhcong" => null]);
                                    }
                                ?>
                                @foreach ($thongbao as $key=>$value)
                                
                                    <div class="card" style="overflow: hidden;">
                                        <a  href="{{URL::to('/Xoathongbao/'.$value->MATHONGTIN)}}" class="close-icon">
                                            <i class="fas fa-times btn btn-outline-primary"></i>
                                        </a>
                                        <h5 class="card-title" style="color: dodgerblue"><img style="width: 32px; height: 32px" src="public/FontEnd/assets/images/notification(1).png" />{{$value->TIEUDE}}</h5>
