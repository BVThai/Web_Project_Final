...
<body data-bs-spy="scroll" data-bs-target=".navbar" data-barba="wrapper">
  ...
  <section id="hero" class="min-vh-100 d-flex align-items-center text-center">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h3 data-aos="fade-left" class="text-uppercase text-white fw-semibold display-1">Lớp học đang bắt đầu</h3>
                @foreach($lophoc as $key=>$tenlop)
                <h4 class="text-white mt-3 mb-4" data-aos="fade-right">{{$tenlop->TENLOPHOC}}</h4>
                @endforeach
                @if ($link_hoc) 
                    <div data-aos="fade-up" data-aos-delay="50">
                        <a href="https://{{$link_hoc->DOMAIN}}/{{$link_hoc->OPTIONS}}" target="_blank" class="btn btn-brand me-2">Tham gia!</a>
                    </div>
                @else
                   
                    <p>Lớp học không tồn tại hoặc đã hết hạn.</p>
                @endif
            </div>
        </div>
    </div>
